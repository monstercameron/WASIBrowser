...
found, moved, monetized, and how they talk to services. "web3" burned the
branding on tokens; none of this uses a coin, a chain, or consensus.
Everything here is hashes, signatures, and caches — boring cryptography, no
speculation.

> **This document is the overview / manifesto / index — not the whole
> normative spec.** It got large enough (a whole platform: naming, bundles,
...
> **⚠ The one experiment that can falsify the whole project (a HARD P0 gate).**
> The bet above only pays off if the identity chrome makes `Google ·
> slate-harbor-vale` feel *as safe and simple* to a non-technical user as
> `google.com` feels today (§10.7). This is the highest-risk item in the plan
> and it is not a protocol problem — it is a UX-research problem. So **P0
> builds and usability-tests the real identity chip + first-contact chooser
> against a control, and does NOT ship global bare-name resolution until users
> demonstrably understand it.** If that chrome can't beat the naive
> domain-plus-lock-icon model in a real study, the core bet is *falsified* —
> and that is the finding, before more protocol code. Everything else is
> downstream of this.
...
1. **Embeddable engine + extension — full verification, capped capabilities.**
   *Framing (set expectations up front): the extension is **"WebNext Preview +
   verified public-bundle reader,"** NOT "WASIBrowser Lite." It shows the
   dream; the native install delivers the paid/private platform. A dev who
   assumes otherwise will build a paid/private flow and be frustrated when the
   extension can't run it.* next.0 execution ships as a spec + a small
   reference engine that plugs into existing browsers as a WASM module /
   extension, so Chrome/Firefox/Safari users can open `web://` links *without
   switching browsers*. **Verification
   ...
   *Host-detection is MANDATORY, not optional:* every host MUST expose a
   read-only `host.trust_tier` (native | extension) so an app can adapt its
   own UI — hide the pay button and show "open in WASIBrowser to purchase"
   under the extension, rather than letting a native-only capability silently
   fail. Making it mandatory (not a nice-to-have) is what lets the two-tier
   reach model work without frustrating developers: the degraded state is a
   first-class, always-detectable fact.
...
- **Data durability is a DECLARED, user-visible capability — the formal fix
  for the dynamic-data weakness.** Every app that stores user-owned state must
  declare a `data_durability` mode in its manifest, and the browser surfaces
  ...
  its failure mode *before* trusting it with years of state, not after the
  operator vanishes. This turns the honest-but-passive "zombie data is the
  operator's problem" into an enforced, comparable, sovereignty-forward
  signal (§0b applied to app state) — and it is the one QoL/security lever that
  most directly addresses next.0's hardest real-world weakness (mutable
  per-user SaaS data). `no-export` is legal but *loud*; the good modes compete.
...
**Generalized in §13 (WVEP).** `payments` is the money case of a larger
browser-mediated value-exchange protocol: an app requesting *value* (money,
attention, compute, storage, bandwidth, or human judgment) and the browser
mediating it under bounds, returning a signed receipt. `payments.charge` is
exactly the PAYMENT class of a `ValueOffer` — read §13 for the full model, of
which this section is the narrowest, most-familiar slice.
...
1. **Human names + the four kingmaker surfaces — now with a concrete
   ...
   guarantee for soft resolution, putting weight on four defaults that could
   silently centralize: the trust-ranking, the commons-namespace set, the R7
   high-risk category list, and the legacy-TLD/ramp table. **Decision (§1c):**
   a top-level defaults manifest references **four independently-signed,
   independently-forkable sub-objects** (never one monolith — that would
   concentrate soft power, letting a single bad actor miscalibrate all four
   and forcing a community disputing *one* to fork *all*). Each is a `web://`
   `ed:` object, forkable like an app update (§11.6), initial content
   published *with the spec*. The default trust-ranking is a **documented,
   criticizable algorithm** — `score = petname-import-frequency +
   log-quorum-age + revocation-history − look-alike-penalty` — not "TBD."
   Residual honest risk: most users never swap defaults, so a default author
   has real soft power — mitigated, not eliminated, by per-sub-object
...
declared `data_durability` mode, §2b)
· **Network** (none · RPC-to-declared-service · declared-gateway-fetch ·
LAN-peer · public-swarm · raw-socket ≈ never) · **Identity** (anonymous ·
app-scoped · contact-share · org-share) · **Value exchange** (WVEP §13 —
declares which of PAYMENT/ATTENTION/COMPUTE/STORAGE/BANDWIDTH/JUDGMENT the app
may request, with per-class caps; `payments` §4a is the PAYMENT slice) ·
**System** (notifications · clipboard r/w · camera+mic · background · device
discovery) · **Execution budgets** (memory · fuel/CPU · background-minutes ·
storage quota · network quota). No raw WASI term ever reaches the user surface.
...
*request value*, and the browser (not the app) decides whether, how, and under
what bounds the user contributes it, returning a signed **receipt**, never
device control. §4a's `payments` is its PAYMENT slice.

> **The non-negotiable invariant: a site may REQUEST value; only the browser
> may EXTRACT value.**

Six value classes → a common signed `ValueReceipt`: **PAYMENT** (money),
**ATTENTION** (a browser-framed sponsor msg, no 3rd-party scripts/tracking),
**COMPUTE** (bounded useful work or CPU-only crypto, in the strictest sandbox),
**STORAGE** / **BANDWIDTH** (verified chunk custody/serving, reusing §2/§3),
**JUDGMENT** (consented human tasks). The flow is offer → bounded session →
receipt; the app verifies the receipt and unlocks, never seeing the user's
identity or device. Crypto generation is CPU-only, battery-off-by-default,
disclosed in trusted chrome, broker-bridged (never direct pool network), with
a "just pay, it's cheaper" greed warning. Brokers can't cross-site-track
(per-site HKDF key, no persistent worker id, **no broker→worker payout leg** —
the contribution *is* the payment). A per-site **Value Ledger** exposes greedy
sites. WVEP's safety invariants *extend* §10.
...
| Term | Meaning |
|---|---|
| `b3:` | a BLAKE3 content hash — the address *is* the bytes; immutable, verify-anywhere (§1) |
| `ed:` | an Ed25519 publisher key — a mutable identity that signs manifests (§1) |
| `name~keytag` | a human label pinned to a key; keytag is word-encoded (`cam~blue-otter-cedar`) and length-scales with claim density (§1a) |
| `@petname` | a name resolved ONLY from your local petname book; never shared (share emits `~keytag`) (§1b) |
| bare name | `web://google` — a *soft* first-contact lookup, C4/not-trusted, resolves to a chooser (§10.3) |
| delegation chain | `google.deepmind.chat` — dots are signed key delegation, root-left; resolved native-first (§1a, §1d) |
| namespace | a `kind:directory` name with a policy-gated admission (e.g. `nyc/hall`); a commons, not a TLD (§1c) |
| defaults sub-objects | four independently-signed/forkable governance lists (trust-ranking, commons namespaces, R7 categories, legacy-TLD table) (§1c) |
| bundle | a content-addressed app: manifest chunk + FastCDC content chunks, merkle-rooted (§2) |
...
The prose + examples above imply these canonical objects; P0 turns each into a
normative schema in its spec-family doc. Listed here so implementation has a
target surface, not just prose:

```
BundleManifest      WEB-BUNDLE   app wasm hash, asset map (path->chunks), entry,
                                 min-ABI, chunking params, kind:app|directory
PublisherManifest   WEB-NAMING   seq, bundle:b3:, prev, title, publisher,
                                 retain, stale_after, sunset, data_ttl,
                                 data_durability, sig
ServiceManifest     WEB-RPC      ed: identity, endpoints, iface:b3:, succession
...
DelegationRecord    WEB-NAMING   parent_key signs {label, child_key, revocable?, exp}
RecoveryRecord      WEB-SECURITY M-of-N | cold-backup | institutional-custody (§10.5)
```

## 16. Happy-path diagrams (the four flows, at a glance)

```
Open a b3: bundle (immutable, offline-capable)
  user --web://b3:hash--> host: resolve (LAN? cache? gateway? swarm?)
                          host: fetch chunks --> VERIFY blake3 root  --(mismatch)--> HARD STOP
                          host: check manifest --> show permissions --> load app.wasm
                          host: run via DOM ABI + cache + compile-cache

Open an ed: publisher app (mutable, updatable)
  user --web://ed:key--> host: resolve manifest (DHT record / dns ramp)
                         host: VERIFY signature --> bundle = b3:... --> (then the b3 path)
                         later: manifest.seq moved? --> BUNDLE_OUTDATED --> onUpdateAvailable
                                update diff (code/publisher/caps/data) --> Auto-safe | Ask | Block

Call an RPC service (location-free, capability-scoped)
  app: gwb.rpc_call(service ed:, iface b3:, method, payload)
       host: check app's value/rpc capability grant
       host: resolve ed: -> endpoints; connect QUIC/Noise; VERIFY service key
       host: send frame (app-scoped caller key) --> RPC_RESULT event back to app
       (service moved hosts / rotated to successor key? client follows + re-verifies)

Complete a WVEP value session (browser-mediated, receipted)
  site: gwb.value_offer(signed offer: pay | attention | compute | ...)
        host: validate offer + user policy --> show REAL cost of each option
        user: picks (e.g. 90s gentle compute)
        host: run bounded ValueSession (metered, killable, Task-Manager-visible)
